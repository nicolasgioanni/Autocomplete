...
private:
  ...
  // Nicolas Gioanni Helper Functions:

  // recursive helper function for copy constructor function
  void copyConstructorHelper(Node *nodeOne, Node *nodeTwo);

  // recursive helper function for clear function
  void clearHelper(Node *node);

  // recursive helper function for contains function
  bool containsHelper(mapped_type &map, Node *node, const key_type &key) const;

  // recursive helper function for inorder function
  void inOrderHelper(Node *node, void visit(const value_type &value)) const;

  // recursive helper function for preorder function
  void preOrderHelper(Node *node, void visit(const value_type &value)) const;

  // recursive helper function for postorder function
  void postOrderHelper(Node *node, void visit(const value_type &value)) const;

  // recursive helper function for rebalance function
  void rebalanceHelper(vector<value_type> &nodes, Node *node);

  // recursive helper function for rebalance and rebalanceHelper function to
  // construct tree
  void treeConstructionHelper(Node *tempNode, vector<value_type> &node,
                              int begin, int end);

  // recursive helper function for getAll function
  void getAllHelper(Node *node, const key_type &key,
                    vector<value_type> &v) const;

  // recursive helper function for operator== function
  bool operatorDoubleEqualHelper(Node *nodeOne, Node *nodeTwo) const;
