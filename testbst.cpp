...
void test04() {
  ...
  BSTMap::value_type item1;
  item1.first = "Hello there";
  item1.second = 1;
  items.push_back(item1);

  BSTMap::value_type item2;
  item2.first = "No";
  item2.second = 2;
  items.push_back(item2);

  BSTMap::value_type item3;
  item3.first = "Bye";
  item3.second = 3;
  items.push_back(item3);

  BSTMap testingFunctions(items);
  BSTMap temp = testingFunctions;

  assert(testingFunctions.contains("No") == 1);
  assert(!testingFunctions.empty());
  assert(testingFunctions.count("Bye") == 1);

  cout << testingFunctions << endl;
